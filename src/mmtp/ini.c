...
{
    struct ini_section *p = (struct ini_section *)calloc(1, sizeof(struct ini_section));
...
{
    struct ini_arg *p = (struct ini_arg *)calloc(1, sizeof(struct ini_arg));
...
*/
    ...
    while ((len = _getline(&line, &n, fp)) != -1)
    {
        ...
        *delimiter = '\0';
        ...
        if (curr == NULL)
        {
            if ((curr = create_section(head, (char*)"global")) == NULL)
            {
                free(line);
    ...
    if (head == NULL)
    {
        if ((head = (struct ini_section *)calloc(1, sizeof(struct ini_section))) == NULL)
            return NULL;
    }
