...
int MP4_ReadBoxCommonFromBuffer( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   if( ( ( i_read = buffer_peek( p_stream, (void*)p_peek, 32 ) ) < 8 ) )
   {
      free((void*)p_buff);
      return 0;
   }
...
static int MP4_ReadBoxSkipFromBuffer( stream_t *p_stream, mp4_box_t *p_box )
{
      ...
      p_box->i_type == ATOM_free )
   {
      ...
      i_read  = buffer_peek( p_stream, (void*)p_peek, 44 );
...
static int MP4_ReadBox_infe( stream_t *p_stream, mp4_box_t *p_box )
{
    ...
    if( i_read > 0 )
    {
        ...
        name = (uint8_t *)malloc( item_name_end_pos + 1 );
        p_box->data.p_infe->item_name = (char*)name; 
		...
		type = (uint8_t *)malloc( content_type_end_pos + 1 );
    p_box->data.p_infe->content_type = (char*)type;
		...
		encoding = (uint8_t *)malloc( i_read + 1 );
    p_box->data.p_infe->content_encoding = (char*)encoding;
...
static int MP4_ReadBox_infeFromBuffer( stream_t *p_stream, mp4_box_t *p_box )
{
	int i;
	uint8_t *name,*type,*encoding;
	uint64_t item_name_end_pos = -1;
	uint64_t content_type_end_pos = -1;
    MP4_READBOX_ENTERFromBuffer( mp4_box_data_infe_t );
...
static int MP4_ReadBox_iinfFromBuffer( stream_t *p_stream, mp4_box_t *p_box )
{
    MP4_READBOX_ENTERFromBuffer( mp4_box_data_iinf_t );

    MP4_GETVERSIONFLAGS( p_box->data.p_iinf );

    MP4_GET2BYTES( p_box->data.p_iinf->entry_count );

    buffer_seek( p_stream, mp4_box_headersize( p_box ) + 6, p_box->i_pos );
    MP4_ReadBoxContainerRaw( p_stream, p_box );
...
static int MP4_ReadBox_iloc( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   for( i = 0; (i < p_box->data.p_iloc->item_count )&&( i_read >= 18 );i++ )
   {
	  ...
	  MP4_GET4BYTES( p_box->data.p_iloc->extent_length[i] );
   }
...
static int MP4_ReadBox_ilocFromBuffer( stream_t *p_stream, mp4_box_t *p_box )
{
   unsigned int i = 0;
   unsigned char offset_size_and_length_size = 0;
   unsigned char base_offset_size_and_index_size_or_reserved = 0;
   MP4_READBOX_ENTERFromBuffer( mp4_box_data_iloc_t );
   MP4_GETVERSIONFLAGS(p_box->data.p_iloc);
   MP4_GET1BYTE(offset_size_and_length_size);
   p_box->data.p_iloc->offset_size=(offset_size_and_length_size>>4)&0x0f;
   p_box->data.p_iloc->length_size=offset_size_and_length_size&0x0f;
...
static int MP4_ReadBox_pitmFromBuffer( stream_t *p_stream, mp4_box_t *p_box )
{
    MP4_READBOX_ENTERFromBuffer( mp4_box_data_pitm_t );
...
static int MP4_ReadBox_idatFromBuffer( stream_t *p_stream, mp4_box_t *p_box )
{
    MP4_READBOX_ENTERFromBuffer( mp4_box_data_idat_t );
...
static int MP4_ReadBox_mdhd( stream_t *p_stream, mp4_box_t *p_box )
{
    ...
    p_box->data.p_mdhd->language_code = SwapBE16( *(uint16_t *)p_peek );
    i_language = p_box->data.p_mdhd->language_code;
    for( i = 0; i < 3; i++ )
    {
...
static int MP4_ReadBox_metaFromBuffer( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   buffer_seek( p_stream, mp4_box_headersize( p_box ) + 4, p_box->i_pos );
...
} MP4_Box_FunctionFromBuffer [] =
{
   ...
   { ATOM_pitm,    MP4_ReadBox_pitmFromBuffer,         MP4_FreeBox_Common},
   { ATOM_iloc,    MP4_ReadBox_ilocFromBuffer,         MP4_FreeBox_iloc},
   { ATOM_iinf,    MP4_ReadBox_iinfFromBuffer,         MP4_FreeBox_Common},
   { ATOM_infe,    MP4_ReadBox_infeFromBuffer,         MP4_FreeBox_infe},
   { ATOM_idat,    MP4_ReadBox_idatFromBuffer,         MP4_FreeBox_idat},
   /* Last entry */
...
mp4_box_t * MP4_BoxSearchBox(mp4_box_t *p_head, uint32_t i_type)
{
    ...
    while (cur != NULL)
    {
        ...
        {
			////printf("2 current box is %c%c%c%c\n",cur->i_type&0x000000ff,(cur->i_type&0x0000ff00)>>8,(cur->i_type&0x00ff0000)>>16,(cur->i_type&0xff000000)>>24);
			if(cur->i_type == i_type)
				{
...
//// printf("this box is %c%c%c%c\n",p_box->i_type&0x000000ff,(p_box->i_type&0x0000ff00)>>8,(p_box->i_type&0x00ff0000)>>16,(p_box->i_type&0xff000000)>>24);

  if(p_box->i_type == 1952539753)  ////decide if the box is idat
	   return NULL;
